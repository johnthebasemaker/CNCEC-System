...
# The shipment's paperwork, selected by EVERY reader of delivery_notes.
#
# One constant rather than four copied column lists, because the requirement
# (2026-08-13) is that the document is visible wherever the delivery is —
# warehouse, logistics, HOD, QC and the receiving store keeper all see the same
# DN from different endpoints. Four hand-maintained lists is how three of them
# end up showing it and the fourth quietly does not.
DN_DOC_COLUMNS = 'dn_document_no, dn_attachment_id, shipped_at, shipped_by'
...
async def ship_dn(session: AsyncSession, *, username: str, dn_number: str,
                  dn_document_no: str = "", attachment_id: int | None = None) -> dict:
    """Mark an HOD-approved DN outbound — WITH its paperwork.

    Both arguments became mandatory on 2026-08-13. A shipment used to leave on
    a bare click, so the site received goods whose physical delivery note had
    no counterpart in the system: our `DN_Number` is generated by us, the
    number printed on the carrier's document is theirs, and nothing tied the
    two together. A disputed delivery was reconciled by telephone.

    The two are refused SEPARATELY rather than as one "paperwork missing"
    error. They are fixed by different actions — one is typing, the other is
    scanning — and an error that names both sends somebody to do the half
    they had already done.

    The MTC upload is deliberately untouched here: it certifies the MATERIAL
    and is inherited down the chain so nobody uploads it twice, and it stays
    exactly as optional at ship time as it was before.
    """
    doc_no = (dn_document_no or "").strip()
    if not doc_no:
        return {"error": "the Delivery Note number printed on the physical "
                         "document is required before shipping — it is what "
                         "lets the site match the paperwork in the cab to this "
                         "record"}
    if attachment_id is None:
        return {"error": "a scan or photo of the signed delivery note must be "
                         "attached before shipping (upload it as a "
                         "'delivery_note' document)"}

    # The attachment must exist, be a delivery note, and belong to the person
    # shipping. Same three questions assert_entry_docs asks of a batch
    # document — asked here rather than there because that gate is per-BATCH
    # and this one is per-SHIPMENT.
    att = (await session.execute(select(
        attachments_t.c["id"], attachments_t.c["doc_type"],
        attachments_t.c["uploaded_by"]
    ).where(attachments_t.c["id"] == int(attachment_id)))).first()
    if att is None:
        return {"error": f"attachment {attachment_id} does not exist"}
    if att[1] != "delivery_note":
        return {"error": f"attachment {attachment_id} is a {att[1]} document, "
                         f"not a delivery note"}
    if att[2] != username:
        return {"error": f"attachment {attachment_id} was uploaded by someone else"}

    # Gate: a DN may only ship once it has cleared BOTH approval stages.
    res = await session.execute(update(delivery_notes_t).where(
        (delivery_notes_t.c["DN_Number"] == dn_number)
        & (delivery_notes_t.c["status"] == "hod_approved")
    ).values(status="in_transit", dn_document_no=doc_no,
             dn_attachment_id=int(attachment_id),
             shipped_at=_dt.datetime.now(), shipped_by=username))
    if res.rowcount == 0:
        return {"error": "DN not found or not HOD-approved yet (submit → logistics → HOD first)"}

    # Link the attachment to the DN so the Document Library and every
    # downstream reader can find it from the shipment rather than only from
    # the uploader's own list.
    dn_id = (await session.execute(select(delivery_notes_t.c["id"]).where(
        delivery_notes_t.c["DN_Number"] == dn_number))).scalar_one()
    await session.execute(update(attachments_t)
                          .where(attachments_t.c["id"] == int(attachment_id))
                          .values(entry_table="delivery_notes", entry_id=dn_id,
                                  doc_number=doc_no))

    await write_audit(session, username, "SHIP_DN", "delivery_notes",
                      f"DN={dn_number} doc={doc_no} attachment={attachment_id}")
...
async def incoming_dns(session: AsyncSession, site_id: str | None):
    ...
    sql = text(f'''
        SELECT "DN_Number", "PO_Number", "Warehouse_ID", "Site_ID", rl_bl_family,
               "DN_Date", "Vehicle_No", "Driver_Name", status, {DN_DOC_COLUMNS}
