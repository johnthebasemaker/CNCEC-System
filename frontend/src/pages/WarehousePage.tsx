...
/**
 * Ship modal (2026-08-13) — a shipment does not leave without its paperwork.
 *
 * Ship used to be a bare button. The truck left, and the site received goods
 * whose physical delivery note existed only on paper in the cab: `DN_Number`
 * is generated by us, the number printed on the carrier's document is theirs,
 * and nothing connected the two. Reconciling a disputed delivery was a phone
 * call.
 *
 * Both fields are required HERE and again in `ship_dn` — this is the half that
 * stops the click, not the half that holds. The MTC upload is untouched and
 * stays optional: it certifies the MATERIAL and is inherited down the chain,
 * so demanding it again at the loading bay is exactly the duplicate upload the
 * inheritance rule exists to prevent.
 */
function ShipModal({ dn, onClose }: { dn: Row | null; onClose: () => void }) {
  const { message } = App.useApp()
  const ship = useShipDn()
  const [docNo, setDocNo] = useState('')
  const [docs, setDocs] = useState<EntryDoc[]>([])

  const close = () => { setDocNo(''); setDocs([]); onClose() }

  const submit = async () => {
    if (!docNo.trim()) { message.warning('Enter the delivery note number'); return }
    if (!docs.length) { message.warning('Attach the signed delivery note'); return }
    try {
      await ship.mutateAsync({
        dn: String(dn!.DN_Number),
        dn_document_no: docNo.trim(),
        attachment_id: docs[0].id,
      })
      message.success(`DN ${dn!.DN_Number} shipped — the site has been notified`)
      close()
    } catch (e) {
      message.error(errMsg(e))
    }
  ...
  return (
    <Modal open={!!dn} title={`Ship ${dn?.DN_Number ?? ''} to ${dn?.Site_ID ?? ''}`}
      onCancel={close} onOk={submit} okText="Ship" confirmLoading={ship.isPending}>
      <Form layout="vertical">
        <Form.Item label="Delivery Note number (on the physical document)" required
          extra="The carrier's or vendor's number — not the DN number above.">
          <Input value={docNo} onChange={(e) => setDocNo(e.target.value)}
            placeholder="e.g. DN-88213 / 4500123456" />
        </Form.Item>
        <EntryDocsUpload docType="delivery_note" siteId={String(dn?.Site_ID ?? '')}
          docNumber={docNo} value={docs} onChange={setDocs} required />
        <Typography.Text type="secondary" style={{ fontSize: 12 }}>
          A Material Test Certificate is optional here and unchanged — upload it
          from Receiving if you hold one. It follows the material to site on its own.
        </Typography.Text>
      </Form>
    </Modal>
  )
...
function DeliveryNotes({ warehouseId }: { warehouseId?: string }) {
  ...
  const columns: ColumnsType<Row> = [
    ...
    { title: 'Status', dataIndex: 'status', key: 'status', render: (v: string) => <Tag>{v}</Tag> },
    {
      ...
      render: (_: unknown, r: Row) => {
        ...
        if (st === 'hod_approved') {
          return <Button size="small" type="primary" onClick={() => setShipping(r)}>Ship</Button>
        }
  ...
  return (
    <>
      ...
      <ShipModal dn={shipping} onClose={() => setShipping(null)} />
    </>
  )
}
