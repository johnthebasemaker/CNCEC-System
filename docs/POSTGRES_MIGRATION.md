...
### 2026-07-30 (HANDOVER · docs consolidation) · actor=interactive · branch=`main` · 🧠 Deployment paused, fine-tuning next
- **New master doc `PROJECT_HANDOVER.md`** at the repo root — the fresh-session entry point: PAST (locked architecture rules), PRESENT (baselines + live CNCEC numbers + caveats), FUTURE (fine-tuning next; Hetzner paused). Every other doc now points at it.
- **Refreshed across the repo:** `PROJECT_STATUS.md` (gates, the 2026-07-27…30 programme, six new locked-rule gotchas, deployment marked PAUSED), `ARCHITECTURE.md` (§1 pg_excel_sync + component identity + the cutover blank-SAP contract, §2 alembic head + engine row, §5 smartTable, §8 suites AW…AZ + the vacuous-parity warning, §9 tunnel hygiene), `REPO_MAP.md` (rules 7/8, pg_excel_sync, 42 specs, 1,276 comparisons), `tools/migration/README.md` (why the SME reseed is REQUIRED — two independent reasons — and what the additive path does instead), `deploy/cloudflared/README.md` (the Error-1033 multi-connector diagnosis + recovery commands + the `ps aux` token exposure), `frontend/README.md` (the smartTable import convention), `tests/e2e/README.md`, `docs/DEBUGGING.md`, `docs/automatic_test.md`, `docs/DEPLOY.md` (pause banner), `docs/NEW_STACK_HANDOFF.md` (marked historical), `handoff.md` (**SME Canon RULE 5** — `(Material_Code, SAP_Code)` identity — plus a note that Canon RULE 2's join description covers the frozen legacy view only).
- **Gates at handover:** service_tests **951/0** (A…AZ) · Playwright **42/42** · parity:sme **1,276** · bug_check **599/0** · derived-view parity **5/5** (fresh cutover) · build+tsc+lint ✅ · alembic single head **`a4e9b1c73f28`** · `gi_database.db` sha256 unchanged.
- **Decision recorded:** Phase 3 (Hetzner Ubuntu + Docker) is **paused, not blocked**; next phase is *Feature Fine-Tuning and UI Polish*.

### 2026-07-27…30 (SME allocation overhaul + global table tools) · actor=interactive · branches=`feat/pg-excel-sync`→`feat/session-report-summary`→`feat/sme-sqm-bottleneck`→`feat/global-table-tools` (PR #15)→`fix/sme-component-pooling` (PR #16)
- **`tools/pg_excel_sync.py`** — single-entry, ATOMIC (one transaction across all five workbook kinds), Postgres-native `ON CONFLICT … DO UPDATE` with `COALESCE(excluded.col, table.col)`, dry-run by default, PG-only guards (scheme-first, so a legitimate `db.example.com` host is not refused), and a `require_project_env()` preflight that names the wrong interpreter instead of dying on `ModuleNotFoundError: fastapi`. Imports `bulk_import.py`'s planners so mapping is never duplicated; **no Pandas** (absent from `backend/requirements.txt`). Fixed a latent ordering bug — recipes must load before equipment or the short-name backfill map is empty on a fresh DB. Suite **AW** (+27). Log: `docs/PG_EXCEL_SYNC_RUNLOG.md`.
- **Session-report aggregation** — "Total Material Demand" leads the workbook, "Material-Wise Summary" leads the PDF, download buttons in Combined Procurement; `to_pdf_sheets(..., page_break_between=)` defaults False so every existing caller stays byte-identical. Suite **AX** (+16). Log: `docs/SESSION_REPORT_SUMMARY_RUNLOG.md`.
- **Two-tier allocation + reverse SQM** (7 operator rulings Q1–Q7) — `Allocated_Qty = Alloc_Available + Alloc_Ordered`; on-order netted `max(Initial_Ordered_Qty − Σreceipts, 0)` (Q2a) against a live double-count (GI-8005762 had 10,920 of 47,320 already received); `Shortfall_Available_Qty` (physical → feasibility) vs `Shortfall_Qty` (net → buy list); `build_sqm_rollup`/`build_sqm_by_code` restate availability as SQM achievable, bottlenecked by the scarcest component; unmodelled codes score 0, never a silent 100% (Q5). New Material-Wise Segregated Report (PDF + Excel). Suite **AY** (+25). Impact then: buy list 386,635 → 159,449 units. Log: `docs/SME_SQM_BOTTLENECK_RUNLOG.md`.
- **Global table sorting + filtering (PR #15, `7868e8d`)** — `frontend/src/lib/smartTable.tsx` wraps antd's `Table` for all **99 grids across 45 files**; sorters/filters derived from the column definitions, zero call-site change, zero added chrome. Server-paginated grids opt out automatically (4 matched). +3 Playwright specs (`table-tools.spec.ts`) → **42/42**; both revert-sabotages fail them. Visual check caught a real defect: the Users grid's Role filter listed raw codes above cells rendering friendly labels — fixed with an explicit `filters` array. Log: `docs/TABLE_TOOLS_RUNLOG.md`.
- **COMPONENT IDENTITY (PR #16, `c62c415`, alembic `a4e9b1c73f28`)** — SME stock pools keyed `(Material_Code, SAP_Code)`, overturning the 2026-07-18 Material_Code pooling rule. The proposed `Material_Code + Material_Name + UOM` key was rejected against the actual workbooks (all four PU rows share name AND UOM; names disagree across workbooks; UOM disagrees on 25/32 pairs). Old grain measured: components A/B reported **fully covered while 10 short**, D **10 short while holding 3× its need**, SQM achievable 0 vs a true 20. Two latent bugs surfaced: `insights.materialMaps` and `sme._demand_matrix` both used `map.set(code, …)`, which *discards* three of four components. Naming fixed per the operator's report (stock master's specific name beats the recipe's generic one; names wrap instead of ellipsing; SAP under the code); `Material_Key` also fixed five grids whose `rowKey="Material_Code"` now collides. Legacy parity restated as a CONSERVATION invariant; a direct suite-AZ test added because that gate is vacuous on the ledger columns. Cutover defect found and fixed (blank-SAP placeholders + the guard protecting the 86 legacy recipe rows). Suite **AZ** (+20 incl. 4 revert-verifications). Live CNCEC after: 32 component pools, achievable now 2,778 m² / with-ordered 24,743 m². Log: `docs/SME_COMPONENT_POOLING_RUNLOG.md`.
...
- **Manuals:** `docs/USER_MANUAL.md` → **v2.1** (Surface-Shields system-first issuing §3.2, Smart Calculator §4.5, handwritten-spec validation + TSV §3.5, deep ask-data search §2.3, sticky-headers/smart-decimals + feedback bullets §1.4, Bug-Tracking triage/prompt §8, SAP-aware import rules + header-name matching §9). Root `SOP.md` gained a v2 pointer note (procedures unchanged); root `USER_MANUAL.md` stays byte-frozen (runtime-parsed by `ai/manual_qa.py` + pinned by bug_check — repoint post-deploy). **RESOLVED 2026-08-24 (Phase 8 slice 8f): the opposite way round — `docs/USER_MANUAL.md` was deleted and the root manual is the single one, brought up to date and repointed in `tools/export_docs_pdf.py`.**
