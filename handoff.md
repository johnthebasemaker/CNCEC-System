...
>
> **RULE 5 — (NEW-STACK, locked 2026-07-30) SME material identity is `(Material_Code, SAP_Code)`.**
> A multi-part chemical system lists ONE `Material_Code` as several distinct physical drums, separated only by the variant SAP — `GI-8005765` is Comp-A/B/C/D at `1041` / `1041-1` / `1041-2` / `1041-3`. On the **new stack** both `sme_recipe` AND `sme_inventory_seed` are keyed on the pair (seed PK widened by alembic `a4e9b1c73f28`), and the allocation engines key every pool, total, shortfall and report row on `Material_Key` = `"CODE|SAP"`. **Never pool by `Material_Code` alone** — it reported covered components as short and short ones as covered, and collapsed SQM-achievable to zero. `Material_Name + UOM` is NOT a usable discriminator (all four PU recipe rows share both). SAP codes are whitespace-normalized on both sides of every join (`"1043 - 2"` ≡ `"1043-2"`). Full rationale: `docs/SME_COMPONENT_POOLING_RUNLOG.md`.
>
> ⚠️ **RULE 2's join description above describes the FROZEN LEGACY SQLite view only.** The legacy `sme_materials_view` reaches the ledger through `inventory.Material_Code`, which sums every variant SAP of a material into one figure. The new stack's `SQL_SME_MATERIALS` reaches it through **the component's own SAP** instead. The isolation rule itself (SME seed never mingles with ERP `inventory`) is unchanged and still binding. Legacy-side parity is now asserted as a CONSERVATION invariant — rolled up to `Material_Code`, every quantity must still match the frozen view.
>
...
> - **🤖 Migration status (FINAL 2026-07-18):** the portability program was superseded — the **new FastAPI+React+PostgreSQL stack is FEATURE-COMPLETE** (see `docs/PROJECT_STATUS.md`); the legacy app is frozen under `legacy/` on SQLite until switch-off. Gates (2026-07-30): service_tests **951/0** · Playwright **42/42** · parity:sme **1,276** · bug_check 599/0. The Hetzner deployment is **PAUSED by decision** in favour of Feature Fine-Tuning and UI Polish — see [`PROJECT_HANDOVER.md`](PROJECT_HANDOVER.md) (runbook when it resumes: `tools/migration/README.md`). *(Historical 2026-07-01 status: interactive build, routine paused, Phase 3A at 10/~55 — see `docs/POSTGRES_MIGRATION.md` §7/§8.)*
...
>    - **🤖 Migration status (FINAL 2026-07-18):** SUPERSEDED — `FRONTEND_GO` was approved 2026-07-04 and the FastAPI+React+PostgreSQL stack is now **FEATURE-COMPLETE** (gates 2026-07-30: 951/0 · 42/42 · 1,276 · 599/0; see `PROJECT_HANDOVER.md`). This coordination box is historical; the Hetzner deployment is PAUSED by decision pending fine-tuning.
